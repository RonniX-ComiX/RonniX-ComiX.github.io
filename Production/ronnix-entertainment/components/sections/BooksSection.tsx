      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto px-4">
           {posts.map((book) => {
             ...
             return (
                <Link to={`/post/${book.id}`} key={book.id} className={`flex flex-col sm:flex-row bg-neutral-900 rounded-xl overflow-hidden border border-neutral-800 hover:border-red-600 transition-colors duration-300 group ${isScheduled ? 'opacity-70 border-yellow-900' : ''}`}>
                ...
                <div className="p-6 flex flex-col justify-between flex-grow relative">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400 mb-3 font-mono border-b border-neutral-800 pb-2">
                        <span className="flex items-center gap-1 text-red-500">
                            <Calendar size={12} /> {formatDate(book.publishedAt)}
                        </span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                            <User size={12} /> {book.authorName || 'RonniX'}
                        </span>
                    </div>

                    <div>
                        <div className="flex items-start justify-between mb-2">
                             <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500 bg-neutral-800 px-2 py-1 rounded">
                    ...
                    <div className="flex items-center justify-end mt-auto pt-4 border-t border-neutral-800/50">
                        <span className="text-red-500 font-bold text-sm uppercase tracking-wider group-hover:underline flex items-center gap-1 transition-all">
